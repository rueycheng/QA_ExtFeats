...
class Layer(object):
    ...
    def output_func(self, input):
        raise NotImplementedError(
            "Each concrete class needs to implement output_func")
...
class FeedForwardNet(Layer):
    def __init__(self, layers=None, name=None):
        ...
        self.num_params = sum(
            [numpy.prod(p.shape.eval()) for p in self.params])
    ...
    def __repr__(self):
        ...
        return '{} [num params: {}]\n{}'.format(self.name, self.num_params,
                                                layers_str)
...
class FastDropoutLayer(Layer):
    ...
    def output_func(self, input):
        mask = self.srng.normal(size=input.shape,
                                avg=1.,
                                dtype=theano.config.floatX)
        return input * mask
...
class LookupTableFast(Layer):
    ...
    def output_func(self, input):
        out = self.W[input.flatten()].reshape(
            (input.shape[0], 1, input.shape[1], self.W.shape[1]))
        if self.pad:
            pad_matrix = T.zeros(
                (out.shape[0], out.shape[1], self.pad, out.shape[3]))
...
class PadLayer(Layer):
    ...
    def output_func(self, input):
        pad_matrix = T.zeros(
            (input.shape[0], input.shape[1], self.pad, input.shape[3]))
...
class MaxOutFoldingLayer(Layer):
    ...
    def __repr__(self):
        return "{}: maxout_size={}".format(self.__class__.__name__,
                                           self.maxout_size)
...
class LinearLayer(Layer):
    ...
    def __repr__(self):
        return "{}: W_shape={} b_shape={} activation={}".format(
            self.__class__.__name__, self.W.shape.eval(), self.b.shape.eval(),
            self.activation)
...
class NonLinearityLayer(Layer):
    ...
    def __repr__(self):
        return "{}: b_shape={} activation={}".format(
            self.__class__.__name__, self.b.shape.eval(), self.activation)
...
class NonLinearityLayerForConv1d(Layer):
    def __init__(self, b_shape, b=None, activation=T.tanh, rng=None):
        ...
        if not b:
            if rng:
                b_values = rng.uniform(size=b_shape).astype(
                    dtype=theano.config.floatX)
            else:
    ...
    def __repr__(self):
        return "{}: shape={} activation={} rng={}".format(
            self.__class__.__name__, self.b.shape.eval(), self.activation,
            self.rng)
...
class MaxPoolLayer(Layer):
    ...
    def __repr__(self):
        return "{}: pool_size={}".format(self.__class__.__name__,
                                         self.pool_size)
...
class ConvolutionLayer(Layer):
    ...
    def __repr__(self):
        return "{}: filter_shape={}; input_shape={}".format(
            self.__class__.__name__, self.W.shape.eval(), self.input_shape)
...
class LogisticRegression(Layer):
    ...
    def f1_score(self, y, labels=[0, 2]):
      ...
      """
        ...
        f1_neg = 2. * prec_neg * recall_neg / T.maximum(1,
                                                        prec_neg + recall_neg)
        ...
        f1_pos = 2. * prec_pos * recall_pos / T.maximum(1,
                                                        prec_pos + recall_pos)
...
class L2SVM(Layer):
    ...
    def output_func(self, input):
        ...
        self.p_y_given_x = T.nnet.softmax(self.f)
        self.y_pred = T.argmax(self.p_y_given_x, axis=1)
        return self.y_pred
    ...
    def hinge_sq(self, y):
        hinge = T.maximum(1. - self.p_y_given_x[T.arange(y.shape[0]), self.y_pred] *
                          (-1)**T.neq(y, self.y_pred), 0.)
        return hinge**2
...
class PairwiseLogisticWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def __repr__(self):
        return "{}: W={}, W_feats={}, b={}, lambda".format(
            self.__class__.__name__, self.W.shape.eval(),
            self.W_feats.shape.eval(), self.b.shape.eval())
    ...
    def output_func(self, input):
        ...
        self.p_y_given_x = T.nnet.softmax(l * dot + (1 - l) * feats_dot +
...
class __PairwiseLogisticWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def output_func(self, input):
        ...
        self.p_y_given_x = T.nnet.softmax(dot + feats_dot + self.b.dimshuffle(
            'x', 0))
        self.prob = self.p_y_given_x[:, -1]
        self.y_pred = T.argmax(self.p_y_given_x, axis=1)
        return self.y_pred
...
class _PairwiseLogisticWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def __repr__(self):
        return "{}: W_pairwise={}, W_q={}, W_a={}, W_feats={}, b={}".format(
            self.__class__.__name__, self.W.shape.eval(),
            self.W_q.shape.eval(), self.W_a.shape.eval(),
            self.W_feats.shape.eval(), self.b.shape.eval())
    ...
    def output_func(self, input):
        ...
        self.p_y_given_x = T.nnet.softmax(dot + feats_dot + T.dot(
            q, self.W_q) + T.dot(a, self.W_a) + self.b.dimshuffle('x', 0))
        self.prob = self.p_y_given_x[:, -1]
        self.y_pred = T.argmax(self.p_y_given_x, axis=1)
        return self.y_pred
...
class PairwiseL2SVMWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def hinge_sq(self, y):
        hinge = T.maximum(
            1. - self.p_y_given_x[T.arange(y.shape[0]), self.y_pred] *
            (-1)**T.neq(y, self.y_pred), 0.)
        return hinge**2
